...
/*
        익명클래스의 경우 클래스 외부의 변수에 접근하기위해서는 그 변수는 final 키워드가 있어야한다.
        * 배열의경우 배열자체가 객체이고 그안의 값의 변경은 array object의 상태변경이라고 생각하면된다.
        * */
public class StreamExample_Parallel {
    public static void main(String[] args) {

        raceConditionTest();

    /*
     * 사용중인 컴퓨터의 코어 개수를 찾는 코드.
     */
        ...
        /**
         * System.setProperty로 parallelism을 설정할 경우 몇 가지 문제가 있다.
         * 1. JVM레벨 설정이므로 예기치 못한곳에서 원치 않는 코어 사용/비사용 문제가 발생할수 있습니다.
         * 2. System.setProperty로 parallelism을 설정하기 전에 Parallel Stream 을 사용할 경우
         *    setProperty로 설정한 값의 영향을 받지 않는것을 확인 됨. 아무래도 이미 생성된 ForkJoinPool을 내부적으로
         *    그대로 사용하기 때문인것 같습니다.
         * 3. 결국 JVM 실행시 옵션으로 설정하시는것이 권장됨.
...
//    System.setProperty("java.util.concurrent.ForkJoinPool.common.parallelism", String.valueOf(numOfCores - 1));
    ...
    private static List<Integer> getNumbers(final int howMany) {
        return IntStream.rangeClosed(1, howMany)
                .mapToObj(i -> i) // auto-boxing을 통해서 int 가 Integer로 자동변환
                .collect(toList());
    }
}
